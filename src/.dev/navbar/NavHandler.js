class NavHandler {
    ...
    */
    generateContent(){
        ...
        let centerPane            = document.getElementById('geotabReformatter');
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'geotabReformatter'){
                    geotab.addin.geotabReformatter.blur();
                } else {
                    geotab.addin.geotabReformatter.focus(global.api, global.state);
                }
            }
        }, 100);
    }
